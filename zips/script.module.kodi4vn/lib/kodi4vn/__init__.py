...
DEFAULT_HEADERS = {
	"User-Agents": CHROME_DESKTOP_AGENTS,
...
def GA(plugin_name, ga_template, args_json):
	try:
		...
		data = {
			...
			'cid': client_id,
			't': 'pageview',
			'dp': (u"{0}{1}".format(plugin_name, page).encode("utf8")),
			'dt': (u"[{0}] - {1}".format(plugin_name, title)).encode("utf8")
		}
		requests.post(ga_url, data=urllib.urlencode(data))
		Log("GA dp: {0}".format(data["dp"]), 1)
